 ...
 * @returns {Promise}
 */
const fallback = ({}, error) => Promise.resolve(error);

/** @typedef {import('moleculer').Context} Context Moleculer's Context */
module.exports = function (name, schemaDefinition, virtualKeys) {
   ...
   * forexample {name:pic, key: "avatar", path: "/"} => localhost:3002/uploads/fabrigeas.jpg
   */
  virtualKeys &&
    virtualKeys.forEach(function (i) {
      schema.virtual(`${i.name}`).get(function () {
  ...
  return {
    ...
    methods: {
      ...
      appendUserIdToQuery(ctx) {
        if (ctx.meta.user) {
          // ctx.params.owner = ctx.meta.user.id;
        }
      },
    },
  };
